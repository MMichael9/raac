...
describe('RAAC Vault Contract', () => {
    ...
    let raacNFT, raacVault, raacCurve
    let usdtWhale
    let dai,usdt,T3crv
    ...
    before(async () => {
        ...
        T3crv = await ethers.getContractAt("IERC20", CURVE_LPTOKEN)
    ...
    describe('Test Add', () => {
        ...
        it('tests the add liquidity function', async() => {
            ...
            console.log('3crv bal: ', await T3crv.balanceOf(raacCurve.address))

            expect(tokens_to_transfer).to.be.equal(await usdt.balanceOf(raacCurve.address))
            expect(0).to.be.equal(await T3crv.balanceOf(raacCurve.address))
            ...
            expect(await T3crv.balanceOf(raacCurve.address)).to.be.greaterThan(97000000000000000000n)
        })
    })
